...
pub fn public_router() -> Router<AppState> {
    Router::new()
        ...
        // GET /api/events/:id/pulse — public, used by frontend for crowd pulse
        .route("/{id}/pulse", get(event::get_event_pulse))
        // GET /api/events/:id/ws — WebSocket for real-time seat and pulse updates
        .route("/{id}/ws", get(event::event_ws_handler))
...
pub fn organizer_router() -> Router<AppState> {
    Router::new()
        .route(
            "/dashboard/summary",
            get(event::get_organizer_dashboard_summary),
        )
        ...
        .route(
            "/{id}/gate-staff",
            get(event::list_assigned_gate_staff).put(event::assign_gate_staff),
        )
